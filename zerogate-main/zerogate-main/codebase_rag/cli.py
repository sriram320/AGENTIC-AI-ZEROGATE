...
@app.command(help="Start the ZeroGate web server with the interactive dashboard.")
def serve(
    ...
    """Start the ZeroGate Web API and Interactive Dashboard."""
    try:
        import uvicorn
    except ImportError:
        app_context.console.print(
            style(
                "uvicorn is not installed. Run: pip install 'zerogate[web]'",
                cs.Color.RED,
            )
        )
        raise typer.Exit(1)
    ...
    app_context.console.print(
        style(
            f"🚀 ZeroGate Web Server starting at http://{host}:{port}",
            cs.Color.GREEN,
        )
    )
    app_context.console.print(
