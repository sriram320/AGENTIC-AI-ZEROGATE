    ...
    def replace_code_block(
        ...
        try:
            ...
            if target_block not in original_content:
                logger.warning("Target block not found directly, falling back to AST-aware function lookup.")
                
                # If we have a function name, extract the exact AST node and replace its span
                function_name = getattr(self, "_last_function_name", None)
                ast_code = None
                if function_name:
                    ast_code = self.get_function_source_code(file_path, function_name)
                    
                if ast_code and ast_code in original_content:
                    modified_content = original_content.replace(ast_code, replacement_block, 1)
                else:
                    # Final fallback: use DMP directly on the whole file using fuzzy patching
                    logger.warning("AST fallback failed. Attempting fuzzy patch application.")
                    diffs = self.dmp.diff_main(target_block, replacement_block)
                    self.dmp.diff_cleanupSemantic(diffs)
                    patches = self.dmp.patch_make(target_block, diffs)
                    patched_content, results = self.dmp.patch_apply(patches, original_content)
                    
                    if not any(results):
                        logger.error("Fuzzy patching failed to apply.")
                        return False
