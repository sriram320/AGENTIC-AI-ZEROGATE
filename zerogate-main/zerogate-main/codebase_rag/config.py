...
class AppConfig(BaseSettings):
    ...
    def _get_default_config(self, role: str) -> ModelConfig:
        ...
        provider = getattr(self, f"{role_upper}_PROVIDER", None) or os.environ.get(f"{role_upper}_PROVIDER")
        model = getattr(self, f"{role_upper}_MODEL", None) or os.environ.get(f"{role_upper}_MODEL")

        if provider and model:
            return ModelConfig(
                provider=provider.lower(),
                model_id=model,
                api_key=getattr(self, f"{role_upper}_API_KEY", None) or os.environ.get(f"{role_upper}_API_KEY"),
                endpoint=getattr(self, f"{role_upper}_ENDPOINT", None) or os.environ.get(f"{role_upper}_ENDPOINT"),
                project_id=getattr(self, f"{role_upper}_PROJECT_ID", None) or os.environ.get(f"{role_upper}_PROJECT_ID"),
                region=getattr(self, f"{role_upper}_REGION", cs.DEFAULT_REGION) or os.environ.get(f"{role_upper}_REGION", cs.DEFAULT_REGION),
                provider_type=getattr(self, f"{role_upper}_PROVIDER_TYPE", None) or os.environ.get(f"{role_upper}_PROVIDER_TYPE"),
                ...
                ) or os.environ.get(f"{role_upper}_SERVICE_ACCOUNT_FILE"),
            )

        # Fallback to orchestrator if specific agent role is not defined, 
        # unless we are already trying to fetch the orchestrator config.
        if role_upper != cs.ModelRole.ORCHESTRATOR.upper():
    ...
    def get_agent_config(self, role: cs.ModelRole) -> ModelConfig:
        """Dynamically fetch the config for any agent role."""
        return self._get_default_config(role)
